...
async function userRoutes(app) {
    ...
    // ✅ Create User
    app.post("/users", { preHandler: (0, role_1.requireRole)(["ADMIN"]) }, async (req, reply) => {
        try {
    ...
    // ✅ Get all users
    app.get("/users", { preHandler: (0, role_1.requireRole)(["ADMIN", "SUPERVISOR"]) }, async (req, reply) => {
        try {
    ...
    app.put("/users/:id", { preHandler: (0, role_1.requireRole)(["ADMIN"]) }, async (req, reply) => {
        try {
            const { id } = req.params;
    ...
    app.get("/workers/:workerId/analytics", { preHandler: (0, role_1.requireRole)(["ADMIN", "SUPERVISOR", "WORKER"]) }, async (req, reply) => {
        try {
            const { workerId } = req.params;
            if (!workerId) {
                return reply.status(400).send({ error: "workerId is required" });
            }
            const worker = await prisma_1.prisma.user.findUnique({
                where: { id: workerId }
            });
            ...
            const recentEvents = tasks
                .flatMap((task) => task.assignmentEvents.map((event) => ({
                id: event.id,
                taskId: task.id,
